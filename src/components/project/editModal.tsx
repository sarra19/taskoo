...
useEffect(() => {
  if (isOpen) {
    if (initialData) {
      ...
      // ✅ Si team est peuplée (array d’objets), on ne garde que les IDs
      const teamIds =
        Array.isArray(initialData.team)
          ? initialData.team.map((member: any) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/project/${initialData._id}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        setErrorMsg(data.message || "Error updating project.");
        return;
      }
                ...
                users.map((user) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => handleTeamSelect(user._id)}
